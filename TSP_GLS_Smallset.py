...
def read_data(path, lines_of_comment=3):
    datafile = open(path, 'r')

    Name = datafile.readline().strip().split()[-1]

    for _ in range(lines_of_comment):
        datafile.readline()

    _ = datafile.readline().strip().split()[-1]
...
def weight_matrix (coord: np.ndarray):
    N = coord.shape[0]
    matrix = []
...
def has_feature(solution, nextcity): # indicate whether the solution exhibit the feature
...
for path in paths:
    ...
    history = np.empty(30)
    for i in range(30):
        opt = guided_local_search(w, eta, max_iter, max_unimproved)
        history[i] = tour_length(w, opt)
